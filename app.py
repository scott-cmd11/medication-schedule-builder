...
if not st.session_state.med_list:
    ...
    for idx, med in enumerate(st.session_state.med_list):
        ...
        # Actions row
        if st.session_state.confirm_delete_id == med.get('added_at'):
            del_col1, del_col2 = st.columns([3, 2])
            with del_col1:
                st.markdown(
                    "<div style='display:flex;align-items:center;height:48px;color:#c2410c;font-weight:600;'>Delete this medication?</div>",
                    unsafe_allow_html=True
                )
            with del_col2:
                y_col, n_col = st.columns(2)
                with y_col:
                    if AppButton("Confirm", key=f"yes_{idx}", type="primary"):
                        ...
                        st.rerun()
                with n_col:
                    if AppButton("Cancel", key=f"no_{idx}", type="secondary"):
                        st.session_state.confirm_delete_id = None
                        st.rerun()
        else:
