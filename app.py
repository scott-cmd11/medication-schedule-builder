            ...
            for med in med_list:
                if slot in med['time_slots']:
                    ...
                    <div class="{card_class}">
                        <div class="med-title">{html.escape(med['name'])}</div>
                        <div class="med-dose">{day_dose} {html.escape(med['strength_unit'])}{dose_indicator}</div>
                    </div>
                    '''
            html += '</td>'
        html += '</tr>'
        ...
        html += f'''
            <tr style="background-color: {bg_color};">
                <td style="padding: 10px; border: 1px solid #e0e0e0;">
                    <strong>{html.escape(med['name'])}</strong>{source_label}<br>
                    <span style="color: #616161;">{med['strength_value']} {html.escape(med['strength_unit'])}</span>
                </td>
        '''
            ...
            if slot in med['time_slots']:
                html += f'''
                    <td style="padding: 10px; border: 1px solid #e0e0e0; text-align: center; background-color: #c8e6c9;">
                        <strong>X</strong><br>
                        <span style="font-size: 0.8rem;">{med['strength_value']} {html.escape(med['strength_unit'])}</span>
                    </td>
                '''
            else:
        ...
        with chip_col:
            st.markdown(f'''
                <div class="selected-chip compact">
                    <span class="selected-chip-name">{html.escape(med_name)}</span>
                </div>
            ''', unsafe_allow_html=True)
        with btn_col:
    ...
    if can_add:
        ...
        <div style="background: var(--success-light); padding: var(--space-2) var(--space-3); border-radius: var(--radius-md); margin-top: var(--space-2); display: flex; align-items: center; justify-content: space-between;">
            <span style="font-size: var(--text-sm); color: var(--gray-700);">
                <strong>{html.escape(medication_name)}</strong> • {st.session_state.dose_value} {html.escape(st.session_state.dose_unit)} • {html.escape(times_str)}
            </span>
        </div>
        ''', unsafe_allow_html=True)
    ...
    for idx, med in enumerate(st.session_state.med_list):
        ...
        <div class="med-item">
            <div class="med-item-info">
                <div class="med-item-name">{html.escape(med['name'])}</div>
                <div class="med-item-details">{html.escape(dose_str)} · {html.escape(times_str)}</div>
            </div>
        </div>
        ''', unsafe_allow_html=True)
