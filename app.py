...
if not st.session_state.med_list:
    ...
    for idx, med in enumerate(st.session_state.med_list):
        ...
        if st.session_state.confirm_delete_idx == idx:
            ...
            with action_col1:
                st.markdown(f'<div style="height: 48px; display: flex; align-items: center; color: var(--danger); font-weight: 500;">Delete {med["name"]}?</div>', unsafe_allow_html=True)

            with action_col2:
                if AppButton("Yes", key=f"confirm_del_{idx}", type="primary"):
            ...
            with action_col3:
                if AppButton("No", key=f"cancel_del_{idx}", type="secondary"):
                    st.session_state.confirm_delete_idx = None
                    st.rerun()
        else:
