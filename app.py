...
if not st.session_state.med_list:
    ...
    for idx, med in enumerate(st.session_state.med_list):
        ...
        # Actions row
        if st.session_state.get('confirm_delete_id') == med.get('added_at'):
            ...
            with action_col2:
                ...
                with c_btn1:
                    if AppButton("Confirm", key=f"conf_del_{idx}", type="primary"):
                        ...
                        st.rerun()
                with c_btn2:
                    if AppButton("Cancel", key=f"canc_del_{idx}", type="secondary"):
                        st.session_state.confirm_delete_id = None
                        st.rerun()
        else:
