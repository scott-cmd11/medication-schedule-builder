...
if not st.session_state.med_list:
    ...
    for idx, med in enumerate(st.session_state.med_list):
        ...
        if st.session_state.confirm_delete_id is not None and st.session_state.confirm_delete_id == med.get('added_at'):
            # Confirmation Mode (Inline)
            col_confirm, col_cancel = st.columns(2)
            with col_confirm:
                if AppButton("✓ Confirm", key=f"confirm_del_{idx}", type="primary", help="Permanently delete"):
                    ...
                    st.rerun()
            with col_cancel:
                if AppButton("✕ Cancel", key=f"cancel_del_{idx}", type="secondary", help="Cancel deletion"):
                    st.session_state.confirm_delete_id = None
                    st.rerun()
        else:
            # Normal Mode
