...
if not st.session_state.med_list:
    ...
    for idx, med in enumerate(st.session_state.med_list):
        ...
        if st.session_state.confirm_delete_id is not None and st.session_state.confirm_delete_id == med.get('added_at'):
            col_msg, col_cancel, col_confirm = st.columns([3, 1, 1])
            with col_msg:
                st.markdown(f'<div style="height: 48px; display: flex; align-items: center; color: var(--danger); font-weight: 500;">Delete this medication?</div>', unsafe_allow_html=True)
            with col_cancel:
                if AppButton("Cancel", key=f"cancel_{idx}"):
                    st.session_state.confirm_delete_id = None
                    st.rerun()
            with col_confirm:
                if AppButton("Delete", type="primary", key=f"confirm_{idx}"):
