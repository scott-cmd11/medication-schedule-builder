            ...
            for med in med_list:
                if slot in med['time_slots']:
                    ...
                    <div class="{card_class}">
                        ...
                        <div class="med-dose">{html_lib.escape(str(day_dose))} {html_lib.escape(str(med['strength_unit']))}{dose_indicator}</div>
                    </div>
                    '''
            html += '</td>'
        html += '</tr>'
    ...
    if can_add:
        ...
        <div style="background: var(--success-light); padding: var(--space-2) var(--space-3); border-radius: var(--radius-md); margin-top: var(--space-2); display: flex; align-items: center; justify-content: space-between;">
            <span style="font-size: var(--text-sm); color: var(--gray-700);">
                <strong>{html_lib.escape(str(medication_name))}</strong> • {html_lib.escape(str(st.session_state.dose_value))} {html_lib.escape(str(st.session_state.dose_unit))} • {html_lib.escape(str(times_str))}
            </span>
        </div>
        ''', unsafe_allow_html=True)
